...
function getFormData(e, callback) {
    var _file = e.target.files[0],
        ...
        _scale = _originSize/_limitSize > 1 ? 1 : 0;

    if (_scale) {
        getOriginImgUrl(e, function(originUrl) {
            compressImgToLimit(originUrl, _limitSize, function(imgBlob) {
...
// 压缩图片至要求占内存大小内
function compressImgToLimit(src, limitSize, callback, scale, lastScale) {
    ...
    compressImg(src, _scale, function (base64) {
        ...
        if (_originSize > limitSize) {
            _scale = (_originSize/limitSize) * _lastScale;
            _lastScale = _scale;
            compressImgToLimit(src, limitSize, callback, _scale, _lastScale);
        } else {
            console.log(_originSize);
...
function compressImg(src, scale, callback) {
    ...
    _img.onload = function (event) {
        ...
        var _dataURL = _canvas.toDataURL('image/jpeg', _compressQua);
